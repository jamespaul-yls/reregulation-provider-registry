...
| DC Rule 5.4(b) | 0 | 0 (no roster exists; permissive ethics rule) | **N/A — structural** | — | — | `validation/dc_rule54.md` |
...
target in every stratified accuracy sample pulled to date. The four zero-provider
programs (CA LDA, DC Rule 5.4(b), TX ALP, WA Entity Pilot) are structural/temporal, not
accuracy gaps — see `docs/sampling_frame.md §3` for the documented reason each one is
correctly zero.
...
```
11 programs, 20 snapshots, 708 providers, 748 events, 0 aliases
```
...
`docs/methodology.md` — this closed a real gap for `prog_dc_rule54` and `prog_tx_alp`
during v1 close-out; both now have a snapshotted evidentiary page).

## 4. Completeness audit (frame reconciliation)

`make completeness` (`completeness/frame_reconcile.py`) — 14 candidate gaps surfaced
against the IAALS external inventory, **14/14 resolved** (0 open):

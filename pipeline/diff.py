...
def diff_snapshots(
    ...
    *,
    ...
    # .date() on an aware datetime returns the calendar date in *whatever offset the
    # object's tzinfo happens to carry* — not a canonical one. retrieved_at can arrive
    # either fresh from a scrape (always UTC) or re-parsed from the exported
    # source_snapshot.csv manifest during `make reproduce` (whatever offset that CSV
    # string was serialized with). Anchoring to UTC explicitly means event_date is the
    # same value regardless of which path produced retrieved_at or what timezone the
    # machine that serialized it was in — e.g. a 2026-06-28 22:04 EDT capture must
    # always compute as 2026-06-29 (its UTC date), never silently as 2026-06-28.
    event_date = new_snapshot.retrieved_at.astimezone(datetime.UTC).date()
    provenance = {
